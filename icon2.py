#!/usr/bin/env python3
# icon2.py - Wersja Stabilna (2025/2026) z dodanymi parametrami T850 i LIFTED
import os
import shutil
import requests
import pandas as pd
...
PARAMS = [
    ...
    "cape_ml", "cin_ml", "vis", "t"  # Dodano "t" dla T850
]
RENAME_MAP = {
    "t2m": "t_2m", "2t": "t_2m",
    "d2m": "td_2m", "2d": "td_2m",
    ...
    "vis": "vis",
    "t": "t_850"  # Dla temperatury na 850 hPa
}
# -----------------------
# FUNKCJE POMOCNICZE
# -----------------------
...
def process_single_param(param, run_date, run_hour):
    ...
    try:
        ...
        if param == "t":
            ds = ds.sel(isobaricInhPa=850, method="nearest")  # Dla T850
        if 'latitude' in ds.coords and 'longitude' in ds.coords:
            ...
            print(" ❌ Nie znaleziono współrzędnych lat/lon")
            return None
        df = point.to_dataframe().reset_index()
    except Exception as e:
        print(f" ❌ Błąd otwarcia xarray/cfgrib: {e}")
        return None
    finally:
        ds.close()
    # 4. Bezpieczne wyciągnięcie czasu i wartości
    time_candidates = ['valid_time', 'time', 'step', 'forecast_time']
    time_col = next((c for c in time_candidates if c in df.columns), None)
    if time_col is None:
        print(" ⚠️ Nie znaleziono żadnej kolumny czasu")
        return None
    if time_col == 'time' and isinstance(df['time'], pd.DataFrame):
        time_series = df['time'].iloc[:, 0]
    else:
        time_series = df[time_col]
    exclude = {'time', 'valid_time', 'step', 'latitude', 'longitude', 'lat', 'lon',
               'number', 'surface', 'heightAboveGround', 'meanSea'}
    data_cols = [c for c in df.columns if c not in exclude and c not in time_candidates]
    if not data_cols:
...
def main():
    ...
    if "t_850" in df.columns: df["T850 [°C]"] = (df["t_850"] - 273.15).round(1)
    ...
    df["LIFTED [J/kg]"] = df.get("cin_ml", 0).round(0).astype(int)  # Użyto CIN jako proxy dla LIFTED (brak bezpośredniego parametru)
    ...
    final_cols = [
        ...
        "WSPD [m/s]", "GUST [m/s]", "WDIR [°]", "CAPE [J/kg]", "LIFTED [J/kg]", "VIS [km]"
    ]
    df = df.rename(columns={"time": "Czas"})
    available_cols = [c for c in final_cols if c in df.columns or c == "Czas"]
