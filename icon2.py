...
# Mapa nazw zmiennych z GRIB (klucze małymi literami) -> nazwa w DataFrame
RENAME_MAP = {
    "t2m": "t_2m", "2t": "t_2m",
    "d2m": "td_2m", "2d": "td_2m",
    ...
    # Kluczowe dla śniegu (sde to standardowa nazwa w cfgrib dla snow depth)
            ...
            with open(fpath, "rb") as f:
                if "icon-tab.csv" in fname:
                    ...
                    try:
                        ...
                        pass # Jeśli folder nie istnieje lub błąd, zostań w głównym
                    f.seek(0)
...
def process_single_param(param, run_date, run_hour):
    ...
    # Pobieramy max wszystkie potrzebne (wersja simplified brała 2, tu weźmiemy wszystkie dla pewności wykresu)
    # Ale jeśli chcesz szybkość wersji simplified, odkomentuj [:2]
    ...
    # 2. Otwieranie w Xarray (NAPRAWIONE: usunięto set_options)
    try:
        # Usunięto problematyczne xr.set_options(...)
        ...
        # Wybór punktu (logika z wersji simplified)
    ...
    data_col = None
    # Najpierw sprawdź mapę nazw
    for col in df.columns:
        if col.lower() in RENAME_MAP:
            data_col = col
            break
            
    # Jeśli nie znaleziono w mapie, weź pierwszą kolumnę niebędącą koordynatą
    if not data_col:
    ...
    if not data_col:
        print(f" ⚠️ Nie znaleziono kolumny danych w DataFrame dla {param}. Dostępne: {df.columns.tolist()}")
        return None
    ...
    std_name = RENAME_MAP.get(data_col.lower(), param) # Użyj nazwy z mapy lub nazwy parametru
...
def main():
    ...
    # Uzupełnianie zerami tam, gdzie to sensowne
    ...
    # -------------------
    # OBLICZENIA (zgodne z simplified)
    # -------------------
    # Temperatura
    ...
    # Śnieg (TO BYŁ PROBLEM - h_snow [m] -> cm)
    ...
    for raw, out in [("clct", "CC"), ("clcl", "CL"), ("clcm", "CM"), ("clch", "CH")]:
        if raw in final_df.columns:
            vals = final_df[raw]
            # Korekta: jeśli dane są 0-1, zamień na procenty
            if vals.max() <= 1.1:
                vals = vals * 100
    ...
    df_export = final_df.rename(columns={"time": "Czas"})
