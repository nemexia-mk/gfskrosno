#!/usr/bin/env python3
# icon2.py - Wersja Stabilna (2025/2026)
import os
import shutil
import requests
import pandas as pd
...
def process_single_param(param, run_date, run_hour):
    ...
    # Najpierw ustalamy kolumnę z czasem (najbardziej prawdopodobne kolejność priorytetów)
    ...
    # Zabezpieczenie przed duplikowaną kolumną 'time'
    if time_col == 'time' and isinstance(df['time'], pd.DataFrame):
        # bardzo rzadki przypadek - bierzemy pierwszą kolumnę
        time_series = df['time'].iloc[:, 0]
    else:
        time_series = df[time_col]

    # Znajdujemy kolumnę z danymi (pomijamy typowe współrzędne)
    ...
    # Bierzemy pierwszą pasującą (najczęściej jest tylko jedna)
    ...
    # Usuwamy duplikaty czasu (na wszelki wypadek)
...
def main():
    ...
    # Wypełnianie brakujących wartości domyślnymi
