...
    """Zwraca kierunek wiatru w skali 16-ramiennej"""
    if np.isnan(deg):
        return "-"
...
    """Określa preferowany typ rotacji superkomórki na podstawie SRH 0-3km.
    Kierunek (Prawoskrętna/Lewoskrętna) pokazywany tylko jeśli ryzyko superkomórki > 20 %.
    W przeciwnym razie: Neutralna.
    """
...
               if not os.path.exists(os.path.join(OUTPUT_DIR, f"krosno_conv_{RUN_DATE}*{RUN_HOUR}z_f{fh:03d}.grib2"))
               or os.path.getsize(os.path.join(OUTPUT_DIR, f"krosno_conv*{RUN_DATE}*{RUN_HOUR}z_f{fh:03d}.grib2")) < 40000]
    if not pending:
        return
...
        local_path = os.path.join(OUTPUT_DIR, f"krosno_conv*{RUN_DATE}*{RUN_HOUR}z_f{fstr}.grib2")
...
    xlsx_path = os.path.join(OUTPUT_DIR, f"krosno_conv*{RUN_DATE}*{RUN_HOUR}z.xlsx")
...
                arch_name = f"gfs_conv*{RUN_DATE}_{RUN_HOUR}.csv"
                with open(p, "rb") as f:
