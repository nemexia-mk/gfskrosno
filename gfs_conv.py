#!/usr/bin/env python3
# gfs_krosno_conv_v2.py - Wersja 2.2 (pełna, naprawiona)
import os
import requests
import xarray as xr
import numpy as np
...
try:
    ...
    print("✅ MetPy załadowane")
except ImportError:
    METPY_AVAILABLE = False
    print("⚠️ MetPy nie zainstalowane")
...
STATIC_MIDDLE = (
    ...
    "&var_CAPE=on&var_CIN=on&var_LFTX=on&var_PWAT=on"
    "&var_HLCY=on&var_DPT=on&var_RH=on&var_SPFH=on&var_VVEL=on"
    "&subregion=on"
...
def safe_get_point(ds, possible_names):
    ...
    for name in possible_names:
        if name in ds.data_vars:
            try:
                return float(np.squeeze(np.array(ds[name].sel(latitude=KROSNO_LAT, longitude=KROSNO_LON, method="nearest"))))
            except:
                continue
    return np.nan
...
def safe_get_point_level(ds, level, possible_names):
    ...
    for name in possible_names:
        if name in ds.data_vars:
            try:
                return float(np.squeeze(np.array(ds[name].sel(level=level, latitude=KROSNO_LAT, longitude=KROSNO_LON, method="nearest"))))
            except:
                continue
    return np.nan

# -----------------------
# POPRAWIONA FUNKCJA PROFILU
# -----------------------
def extract_profile_for_metpy(ds_isobaric):
    if ds_isobaric is None or not METPY_AVAILABLE: return None
    try:
        level_name = 'level' if 'level' in ds_isobaric.coords else 'isobaricInhPa' if 'isobaricInhPa' in ds_isobaric.coords else [k for k in ds_isobaric.coords if 'hPa' in str(k).lower()][0]
        levels = ds_isobaric[level_name].values.astype(float)
        idx = np.argsort(-levels)
        levels_sorted = levels[idx]
        p = levels_sorted * units.hPa
        t = (ds_isobaric['t'] if 't' in ds_isobaric.data_vars else ds_isobaric['TMP']).sel({level_name: levels_sorted}).values * units.K
        td = ds_isobaric['dpt'].sel({level_name: levels_sorted}).values * units.K if 'dpt' in ds_isobaric.data_vars else t - 8 * units.K
        u = ds_isobaric['u'].sel({level_name: levels_sorted}).values * units('m/s')
        v = ds_isobaric['v'].sel({level_name: levels_sorted}).values * units('m/s')
        hgt = (ds_isobaric['gh'] if 'gh' in ds_isobaric.data_vars else ds_isobaric['HGT']).sel({level_name: levels_sorted}).values * units.meter
...
def calc_ehi(cape, srh): return round((cape * srh) / 160000, 2) if not np.isnan(cape) and not np.isnan(srh) and cape > 0 else 0.0
...
def classify_storm_mode(cape, dls06, srh3, cin, lcl, prob):
    if prob < 15 or np.isnan(cape) or cape < 100: return "Brak / Słaba"
...
# -----------------------
# POBIERANIE (BRAKUJĄCA FUNKCJA - TERAZ DODANA)
# -----------------------
def download_missing_gribs_parallel(forecast_hours):
    pending = [fh for fh in forecast_hours
               ...
               or os.path.getsize(os.path.join(OUTPUT_DIR, f"krosno_conv_{RUN_DATE}_{RUN_HOUR}z_f{fh:03d}.grib2")) < 45000]
...
def process_local_gribs(forecast_hours):
    ...
    for fh in forecast_hours:
        ...
        try:
            ...
            wdir = (270 - np.rad2deg(np.arctan2(v10, u10))) % 360 if not np.isnan(u10) else np.nan
            foehn = 157.5 <= wdir <= 247.5 if not np.isnan(wdir) else False
            ...
            if profile and METPY_AVAILABLE:
                ...
                try:
                    ...
                    u_storm, v_storm, _ = mpcalc.bunkers_storm_motion(p, u, v, hgt)
                    srh01_val = mpcalc.storm_relative_helicity(hgt, u, v, depth=1*units.km, storm_u=u_storm, storm_v=v_storm)[0]
                    srh03_val = mpcalc.storm_relative_helicity(hgt, u, v, depth=3*units.km, storm_u=u_storm, storm_v=v_storm)[0]
            ...
            storm_mode = classify_storm_mode(cape, dls06, srh3, cin, 125*(t2m - (td850 if not np.isnan(td850) else t2m-8)), prob_temp)
            ...
            prob = min(100, max(0, int(round(min(cape / 1100, 1) * 32 + (15 if -80 < cin < -10 else 0) + (12 if dls06 > 18 else 0) + (10 if srh3 > 120 else 0) + (8 if pwat > 32 else 0) + (6 if lr_700_500 > 7.0 else 0)))))
            if foehn:
                prob = int(prob * 0.65)
            ...
            rows.append({
                ...
                "STP (pełny)": stp_full if not np.isnan(stp_full) else "-",
                "PWAT [mm]": round(pwat, 1) if not np.isnan(pwat) else "-",
                "LCL [m]": round(125 * (t2m - (td850 if not np.isnan(td850) else t2m-8)), 0),
...
def save_outputs(df):
    ...
    with pd.ExcelWriter(xlsx_path, engine='xlsxwriter') as writer:
        ...
        ws.conditional_format('F2:F300', {'type': 'cell', 'criteria': '>=', 'value': 1.2, 'format': red})
        ws.conditional_format('H2:H300', {'type': 'cell', 'criteria': '>=', 'value': 1.0, 'format': red})
        ws.conditional_format('P2:P300', {'type': 'cell', 'criteria': '>=', 'value': 70, 'format': red})
...
def upload_to_ftp(files):
    ...
    try:
        ...
        for p in files:
            ...
            with open(p, "rb") as f: ftp.storbinary(f"STOR {target}", f)
        ftp.quit()
        print("✅ FTP OK")
    except Exception as e:
        print(f"❌ FTP Error: {e}")
...
if __name__ == "__main__":
    print(f"\n=== GFS CONVECTION v2.2 {RUN_DATE}{RUN_HOUR}Z ===\n")
