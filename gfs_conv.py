#!/usr/bin/env python3
# gfs_conv_v9_ultimate.py - OSTATECZNA WERSJA (BEZ BŁĘDU 500, DEBUG, FIX PAMIĘCI)
import os
import requests
import xarray as xr
import numpy as np
...
try:
    ...
    print("[INFO] Biblioteka MetPy nie jest zainstalowana. Używam tylko standardowych wzorów.", flush=True)
...
elif time(3, 0) <= current_time < time(10, 30):
    RUN_HOUR = "00"
...
# BEZPIECZNY ADRES URL (Nie powoduje błędu 500 po godzinie f240)
STATIC_MIDDLE = (
...
def calc_srh_01(u10, v10, u925, v925, u_storm, v_storm):
    if any(np.isnan(x) for x in [u10, v10, u925, v925, u_storm, v_storm]): return np.nan
...
def robust_lcl(t2m_c, td850_c):
    if np.isnan(t2m_c): return 1000.0
    if np.isnan(td850_c): td850_c = t2m_c - 8.0
    diff = max(0.0, t2m_c - td850_c)
    return float(np.clip(round(125.0 * diff, 0), 200, 3000))

# ==================== BEZPIECZNE POBIERANIE (Z RETRY I 1 WĄTKIEM) ====================
def download_missing_gribs_parallel(forecast_hours):
    ...
    def fetch_single(fh):
        ...
        for attempt in range(max_retries):
            sleep(1.5) # Podstawowe opóźnienie między KAŻDYM plikiem
            try:
                ...
                if r.status_code == 200:
                    if b"GRIB" in r.content[:12]:
                        with open(local_path, "wb") as f:
                            f.write(r.content)
                        print(f"  ✅ [SUKCES] Pomyślnie pobrano f{fh:03d}", flush=True)
                        return True
                    else:
                        print(f"  ⚠️ [ZŁY FORMAT] f{fh:03d} -> Pobrany plik to nie GRIB.", flush=True)
                elif r.status_code == 403:
                    print(f"  ⚠️ [BLOKADA 403] f{fh:03d} -> NOAA blokuje (próba {attempt+1}/{max_retries}). Czekam 10s...", flush=True)
                    sleep(10) # Kara czasowa za 403, czekamy aż zapora NOAA ostygnie
                else:
                    print(f"  ❌ [BŁĄD] f{fh:03d} -> Kod HTTP: {r.status_code} (próba {attempt+1}/{max_retries})", flush=True)
                    sleep(5)
            except Exception as e:
                print(f"  🚨 [WYJĄTEK HTTP] f{fh:03d} -> {e}. Czekam 5s...", flush=True)
                sleep(5)
                
        print(f"  🛑 [PORAŻKA] f{fh:03d} -> Nie udało się pobrać pomimo {max_retries} prób.", flush=True)
        return False

    # Używamy tylko 1 wątku! NOAA bardzo rygorystycznie podchodzi do połączeń równoległych z tego samego IP.
    with ThreadPoolExecutor(max_workers=1) as ex:
        ex.map(fetch_single, pending)

# ==================== PRZETWARZANIE (Z OCHRONĄ PAMIĘCI) ====================
def process_local_gribs(forecast_hours):
    ...
    for fh in forecast_hours:
        ...
        try:
            ...
            datasets.extend([ds_sfc, ds_accum, ds_2m, ds_10m, ds_pwat, ds_isobaric, ds_hlcy, ds_500, ds_925, ds_850, ds_700])
            ...
            # Logika Opadowa (Najpierw Accum, jak nie ma to fallback)
            apcp = safe_get_point(ds_accum, ['apcp', 'APCP'])
            if np.isnan(apcp):
            ...
            # FIX PAMIĘCI - KLUCZ DO ZAPOBIEGANIA BŁĘDOWI 'TOO MANY OPEN FILES'
            for ds in datasets:
                if ds is not None:
                    ds.close()
